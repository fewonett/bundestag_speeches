...
def save_to_s3(filename, text, s3, bucket_name):
    """This function takes in a filename and text and saves the file as an xml in the s3"""
    try:
        s3.put_object(
            Bucket=bucket_name, Key=filename, Body=text, ContentType="application/xml"
        )
...
def fetch_and_upload_new_xml(newest_date, s3, bucket_name):
    ...
    BUNDESTAG_KEY = os.environ["BUNDESTAG_KEY"]  # Calm down, key is public
    URL = f"https://search.dip.bundestag.de/api/v1/plenarprotokoll?f.zuordnung=BT&f.datum.start={newest_date}"

    headers = {"Authorization": f"ApiKey {BUNDESTAG_KEY}"}
    ...
    if response.status_code != 200:
        raise Exception(
            f"Something went wrong with the Bundestag API. Response Code: {response.status_code}"
        )

    res = response.json()
    if res["numFound"] > 100:
        raise Exception(
            "More than 100 new documents - cursor logic implementation necessary: See Bundestag API docs"
        )
    ...
    for doc in docs:
        try:
            ...
            print(f"Most likely XML file was not added yet. Error: {e}")
        doc_nr = doc_dict["doc_nr"].replace("/", "_")
        date = doc_dict["date"]
        response = requests.get(doc_dict["xml_url"])
        if response.status_code != 200:
            raise Exception(
                f"Could not fetch XML file: Status code: {response.status_code}"
            )
...
# Todo Refactor:
# Find better way to figure out start date --> Annoying to read all file names each time
# Bugfix: Corner case when there is a new protokoll but the xml is not uploaded yet leads to error
